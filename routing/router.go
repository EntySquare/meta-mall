...
func AppSetUp(appApi fiber.Router) {
	...
	appApi.Post("/insertNft", intcpt.AuthManagerApp(), app.InsertNft)
	appApi.Post("/tokenIdFrom", intcpt.AuthManagerApp(), app.TokenIdFrom)
	appApi.Post("/loginManager", app.LoginManager)
	appApi.Post("/managerNftlist", intcpt.AuthManagerApp(), app.GetManagerNftList)
